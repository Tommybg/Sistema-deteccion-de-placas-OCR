...
Tabs:
  1. Pipeline completo  — sube una imagen y corre los 4 modelos en secuencia
...
MODELS_CONFIG = {
    ...
    "ocr": {
        "file": None,
        "edgetpu": None,
        "label": "🔤 Texto de la placa (OCR)",
        "hint": "cpu_only", "scale": 1,
        "desc": "CCT-XS (Compact Convolutional Transformer) de fast-plate-ocr. Lee caracteres alfanuméricos en la placa. Corre en CPU vía ONNX Runtime (~3ms) — el transformer no es compatible con Edge TPU.",
    },
}
...
with st.sidebar:
    ...
    **Modos de ejecución:**
    ...
    - 🟠 **OCR** = ONNX Runtime CPU (transformer ≠ Edge TPU)
    """)
...
st.markdown(
    "Testa los **4 modelos INT8** del pipeline ANPR en modo simulación Coral. "
    "Los resultados son idénticos a ejecución real en hardware Coral Edge TPU."
)
st.divider()
...
with tab_pipeline:
    st.markdown("""
    **¿Qué hace este tab?**
    Corre los 4 modelos en secuencia sobre la imagen que subas, igual que el pipeline ANPR real:
    ...
    4. 🔤 **OCR de placa** — lee los caracteres alfanuméricos (CCT-XS vía ONNX Runtime)

    Muestra los resultados de cada modelo + las latencias reales medidas en tu CPU,
    con el estimado de cuánto tardaría en Coral Edge TPU real.
    """)
    ...
    if img_source:
        ...
        if st.button("▶ Ejecutar Pipeline ANPR Coral", use_container_width=True):
            ...
            with st.spinner("Corriendo los 4 modelos..."):
                ...
                ocr_obj = load_ocr()
                if ocr_obj:
                    # Recortar la zona inferior-central donde suele estar la placa
                    h, w = frame_rgb.shape[:2]
                    plate_crop = frame_rgb[int(h * 0.55):, int(w * 0.15):int(w * 0.85)]
                    plate_resized = cv2.resize(plate_crop, (128, 64))
                    t0 = time.perf_counter()
                    try:
                        res = ocr_obj.run(plate_resized)
            ...
            with col_results:
                ...
                if veh.get("name"):
                    ...
                    result_card("Tipo de vehículo", "No detectado", "Confianza < 25% — intenta con imagen más clara", "🚫")
                ...
                if ocr_det:
                    ...
                    result_card("Texto de placa (OCR)", txt if txt else "—", "Recorte zona inferior-central de la imagen", "🔤")
                ...
                total_tpu = sum(v[1] for v in latencias.values() if v[1] != v[0])
                total_tpu += sum(v[0] for v in latencias.values() if v[1] == v[0])  # OCR stays CPU
...
with tab_individual:
    ...
    **¿Qué hace este tab?**
    ...
    model_sel = st.selectbox(
        ...
        options=["vehicle", "color", "brand", "ocr"],
        ...
        if st.button(f"▶ Ejecutar {cfg_sel['label']}", key="run_ind"):
            ...
            if model_sel == "ocr":
                ...
                if ocr2:
                    ...
                    plate_img = cv2.resize(frame2_rgb[int(h*0.55):, int(w*0.15):int(w*0.85)], (128, 64))
                    t0 = time.perf_counter()
                    try:
                        res = ocr2.run(plate_img)
                    ...
                    st.caption("💡 El OCR recorta la zona inferior-central de la imagen (donde suele estar la placa) y la redimensiona a 128×64 antes de clasificar.")
                    st.image(plate_img, caption="Crop enviado al OCR (128×64)", width=256)
                else:
            ...
            # ── TFLite models ─────────────────────────────────────────────────
                ...
                # ── Brand / Vehicle: top detección ───────────────────────────
                    ...
                    if cls_id is not None and conf > 0.1:
                        ...
                        st.warning("Sin detecciones con confianza > 10% en esta imagen. Intenta con otra foto.")
...
with tab_benchmark:
    ...
    **¿Qué hace este tab?**
    ...
    if st.button("▶ Ejecutar benchmark completo", use_container_width=True, key="run_bench"):
        ...
        total_steps = len(available) + 1  # +1 for OCR

        for i, (key, interp_data) in enumerate(available):
            cfg = MODELS_CONFIG[key]
            progress.progress((i) / total_steps, text=f"Benchmarking {cfg['label']}...")
        ...
        # OCR benchmark
        progress.progress(len(available) / total_steps, text="Benchmarking OCR...")
        ocr3 = load_ocr()
        ocr_mean = None
        if ocr3:
